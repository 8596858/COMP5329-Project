...
class Channel_Mamba_Block(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        """
        B, N, C = x.shape
        v = self.qkv(x).reshape(B, N, 1, self.num_heads, C // self.num_heads)
        v = v.permute(2, 0, 3, 1, 4)
        v = v.transpose(-2, -1)

        v_ = v.reshape(B, C, N).contiguous().view(B, C, H, W)

        attened_x = self.mamba(x)
...
class hyper_state_32_MCRM(nn.Module):
    """ Mamba Pixel-wise Sequential Interaction Network
    Args:
        img_size (int): Input image size. Default: 64
        in_chans (int): Number of input image channels. Default: 3
        embed_dim (int): Patch embedding dimension. Default: 180
        depths (tuple(int)): Depth of each residual group (number of SAMB in each RG).
        split_size (tuple(int)): Height and Width of spatial window.
